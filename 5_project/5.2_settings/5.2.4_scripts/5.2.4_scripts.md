...
- `property.type` — тип параметра (string, boolean, select, multiselect, link, lookup, date, datetime);
- `property.old_value` — текущее значение (nil, если параметр не был заполнен);
- `property.new_value` — новое значение.

Значения конвертируются так же, как в `params:getProp` (boolean — true/false, "дата и время" — число unix time, "список (несколько значений)" — таблица строк, пустое значение — nil). Для параметров-справочников передаётся отображаемое значение строки справочника.
